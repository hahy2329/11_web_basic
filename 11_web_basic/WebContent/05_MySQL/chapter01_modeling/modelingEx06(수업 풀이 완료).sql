...
INSERT INTO TEST5(PRODUCT_CD , PRODUCT_NM) VALUES(6 , '상품6'); 	#SUCCES			 
...
INSERT INTO TEST6_2 VALUES ('O6' , '유저1' , 4); #메인테이플과 참조테이블사이의 데이터 차이가 발생 
INSERT INTO TEST6_2 VALUES ('O7' , '유저2' , 5);	#메인테이플과 참조테이블사이의 데이터 차이가 발생 

UPDATE TEST6_1 SET PRODUCT_CD = 11 WHERE PRODUCT_CD = 1; #메인테이플과 참조테이블사이의 데이터 차이가 발생 
UPDATE TEST6_1 SET PRODUCT_CD = 22 WHERE PRODUCT_CD = 2;#메인테이플과 참조테이블사이의 데이터 차이가 발생 
DELETE FROM TEST6_1 WHERE PRODUCT_CD = 3;     #메인테이플과 참조테이블사이의 데이터 차이가 발생 
...
UPDATE TEST7_1 SET PRODUCT_CD = 11 WHERE PRODUCT_CD = 1; #참조데이터가 있을 경우 메인키와 데이터만 수정 불가 
UPDATE TEST7_1 SET PRODUCT_CD = 22 WHERE PRODUCT_CD = 2;#참조데이터가 있을 경우 메인키와 데이터만 수정 불가 
DELETE FROM TEST7_1 WHERE PRODUCT_CD = 3;   #참조데이터가 있을 경우 메인키와 데이터만 수정 불가            
...
CREATE TABLE TEST8_2(
    ...
    MEMBER_ID VARCHAR(20),
    PRODUCT_CD INT,
	#FOREIGN KEY (PRODUCT_CD) REFERENCES TEST8_1(PRODUCT_CD)
