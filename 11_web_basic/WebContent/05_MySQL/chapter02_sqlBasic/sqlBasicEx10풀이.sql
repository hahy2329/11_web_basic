...
SELECT COUNT(*)
FROM ORDER_TB
WHERE MEMBER_ID IN('USER1', 'USER2', 'USER3');
...
SELECT SUM(ORDER_GOODS_QTY)
FROM ORDER_TB
WHERE PRODUCT_CD IN('product1' , 'product3' , 'product5');
...
# 배송메세지별로 개수를 조회하기
 ...
 SELECT DELIVERY_MESSAGE, COUNT(DELIVERY_MESSAGE)
 FROM ORDER_TB
...
# 배송완료가 아닌 배송상태별로 개수를 조회하기 
  ...
  SELECT DELIVERY_STATUS, COUNT(DELIVERY_STATUS)
  FROM ORDER_TB
  WHERE DELIVERY_STATUS<>'배송완료'
  GROUP BY DELIVERY_STATUS;
...
FROM ORDER_TB
GROUP BY MEMBER_ID, PRODUCT_CD
ORDER BY MEMBER_ID, PRODUCT_CD;
...
/*
	 ...
	 # HAVING
        ...
        GROUP BY 
			<필드명> 
		HAVING 
			<GROUP BY조건>   //직계함수 즉, SELECT 구문에 SUM, MAX, COUNT 등등에 관한 조건일 때 쓰임 
    
 */	

# 상품별로 주문 총수량을 조회하되 판매량이 10개 이상인 상품만 조회하기

SELECT PRODUCT_CD, SUM(ORDER_GOODS_QTY) AS TOTAL_ORDER_GOODS_QTY	# SUM(ORDER_GOODS_QTY)
FROM ORDER_TB
GROUP BY PRODUCT_CD
HAVING 	TOTAL_ORDER_GOODS_QTY>=10;				# SUM(ORDER_GOODS_QTY) >= 10;
