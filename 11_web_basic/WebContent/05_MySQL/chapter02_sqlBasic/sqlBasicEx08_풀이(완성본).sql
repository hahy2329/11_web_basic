...
SELECT PRICE, PRICE * 0.1 AS ADDITIONAL_TAX, PRICE + PRICE *01 AS TOTAL_PRICE
FROM BOOK;
...
SELECT PRICE, DISCOUNT_RATE, PRICE- (PRICE * DISCOUNT_RATE)/100 AS  TOTAL_PRICE
FROM BOOK;
...
SELECT PRICE, DISCOUNT_RATE, PRICE * 0.1 AS ADDITIONAL_TAX, PRICE - (PRICE * DISCOUNT_RATE) / 100 + PRICE * 0.1 AS TOTAL_PRICE 
FROM BOOK;
...
DELETE FROM BOOK
WHERE TITLE IN('컴퓨터활용능력', '초등학생이 알아야할 숫자');
