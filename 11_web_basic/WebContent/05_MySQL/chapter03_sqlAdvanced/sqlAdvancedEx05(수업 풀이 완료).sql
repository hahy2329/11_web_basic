...
FROM CAR_BAK;
 ...
 INSERT INTO CAR_BAK (SELECT * FROM CAR); #이런게 있다라고만 알고 있자!
...
DELETE FROM CAR WHERE CAR_CD NOT IN(SELECT CAR_CD
FROM RENT);
...
# 2) 스칼라 서브쿼리 예시 ----> 스칼라는 SELECT문(행1개)에 들어간다. 
# 'k5' 차량의 렌트가격에서 전체 차량의 평균 렌트가격을 뺀 가격을 조회하기



SELECT PRICE - AVG(PRICE) # ERROR에러 
FROM CAR 
WHERE CAR_NM = 'k5';
...
SELECT ROUND(AVG(T1.RENT_PERIOD)) 
FROM (SELECT *
	  FROM RENT
	  WHERE CAR_CD IN(SELECT CAR_CD 
						FROM CAR 
						WHERE BRAND_NM = '현대')) AS T1;
