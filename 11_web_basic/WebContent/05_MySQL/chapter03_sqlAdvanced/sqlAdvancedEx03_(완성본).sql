...
SELECT ROUND(AVG(PRICE)) AS AVANTE_AVG
FROM CAR_TB C
		INNER JOIN RENT_TB R 
				ON C.CAR_CD = R.CAR_CD
                AND C.CAR_NM = '아반떼';
...
SELECT ROUND(AVG(R.RENT_PERIOD)) AS MAN_RENT_PERIOD
FROM RENT_TB R 
			INNER JOIN MEMBER_TB M 
					ON R.MEMBER_ID = M.MEMBER_ID
                    AND M.SEX ='m';
...
SELECT
		SUBSTRING(C.REG_DT , 1 , 4) AS YEAR,
        M.SEX AS SEX,
		ROUND(AVG(R.RENT_PERIOD))  					AS RENT_PERIOD_AVG
FROM
	 RENT_TB R
  INNER JOIN CAR_TB C
		  ON R.CAR_CD = C.CAR_CD
...
SELECT C.CAR_CD, C.CAR_NM, COUNT(*) AS RENT_CNT
FROM RENT_TB R 
		INNER JOIN CAR_TB C
				ON R.CAR_CD = C.CAR_CD
                
GROUP BY C.CAR_CD, C.CAR_NM
...
SELECT M.MEMBER_ID, M.MEMBER_NM, COUNT(*) AS RENT_CNT
FROM RENT_TB R
		INNER JOIN MEMBER_TB M 
				ON R.MEMBER_ID = M.MEMBER_ID
